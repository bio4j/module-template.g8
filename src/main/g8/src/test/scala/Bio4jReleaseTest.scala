...
object TestBundles {
  ...
  case object TestBundle 
    extends ReleaseBundle($name;format="word"$Release.s3address, $name;format="word"$Release.module) {
      override def install[D <: AnyDistribution](d: D): InstallResults = {
        checkGorilla(new NodeRetrieverTitan(
            new Bio4jManager($name;format="word"$Release.module.dbLocation.getAbsolutePath))) -&-
        $name;format="word"$Release.install(d) -&-
        $name;format="word"$Distribution.install(d) -&-
        checkGorilla($name;format="word"$Distribution.nodeRetriever)
      }
  }
